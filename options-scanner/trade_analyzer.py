...
Version: 1.1.0
Last Updated: 2026-09-19 (restored; events from the earnings calendar)
...
def _check_scenario_strike_proximity(trade, scenarios):
    ...
    for sc in scenarios:
        if _structures.is_put_side(strategy) and "\u22121\u03c3" in sc["label"]:
            pin = trade.get("long_strike")  # the spread's long put
            if pin and _near_pin(sc["price"], pin, "below"):
                warnings.append(
...
def _collect_event_context(trade, dte_remaining, quote_data, earnings_date=None):
    """Return ``{"events_in_window": [...]}`` for the position's remaining life.

    The only event source this app still keeps is the earnings calendar
    (``shared/earnings.py``); the macro calendar module this once read was
    deleted on 2026-08-21, which is how this path came to raise on every call.
    ``earnings_date`` is INJECTED (a ``date`` or ISO string, or None) so the
    caller owns the SQLite read -- the same shape as Rescue's earnings factor.
    ``quote_data`` is accepted for call compatibility and unused.
    """
    ...
    if isinstance(ed, datetime):
        ed = ed.date()
    if isinstance(ed, date) and today <= ed <= exp:
        events.append({
            "date": ed.isoformat(),
            "label": f"{trade.get('symbol', '')} earnings".strip(),
            "category": "EARNINGS",
            "source": "earnings_calendar",
            "days_until": (ed - today).days,
        })
    return {"events_in_window": events}
...
def analyze_trade(client, trade, iv_data=None, earnings_date=None):
    ...
    Args:
        ...
        earnings_date: the symbol's next report (date / ISO string) or None
