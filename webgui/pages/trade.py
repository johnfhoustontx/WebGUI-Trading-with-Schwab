...
# (``TYPICAL_ANALYZE_SEC`` / ``ANALYZE_TIMEOUT_SEC`` / ``analyzing_label`` moved
# to ``pages/trade_shell.py`` on 2026-09-20. They were written for THIS page -- a
# page nothing routes -- while the four reachable Signal Desk screens ran
# ``busy.BUSY_TIMEOUT_SEC``'s 30s default against the same measured 96s
# analysis. ``render`` below is deleted in Task 6 of the UI-consistency phase,
# which is why it reaches them through a function-local import rather than the
# module-level one that would close the trade <- trade_shell import loop.)


def render():
    """Trade page: symbol input + Analyze button + verdict/MTF/momentum cards."""
    from pages.trade_shell import ANALYZE_TIMEOUT_SEC, analyzing_label
