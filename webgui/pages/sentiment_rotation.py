...
``kit.region`` covers the whole board — the gauge, its axis, the flow band, its
footer and the quadrant panels are all replaced together by a refresh. The
page's own ground, face and warm-neutral ladder are gone; it wears the app
surface like every other screen. ⚠ The DATA colours are untouched and must stay
so: every ``TONE`` entry (the regime dot and word, the gauge fill and mark, the
trigger ticks, the flow footers) and every ``quad_classes`` key (the band
segments, the panel dots and titles, the chips and their bars) encodes a value.
Panel hairlines, the quadrant grid, the gauge groove, the zero tick and the chip
bar troughs are chart FURNITURE and take app border tokens.
"""
import bus_client
...
# Chart furniture, not data. A hairline, the quadrant grid's gap colour, the
# gauge's unfilled groove and a chip's bar trough all draw a FRAME, so they take
# the app's card border. The gauge's ZERO tick sits one step brighter, on the
# button border, for the same reason the RRG's crosshair does: it marks the
# plot's one fixed reference, where the groove is only a scale.
_HAIRLINE = f"bg-[{_P['card_border']}]"
_FRAME_EDGE = f"border-[{_P['card_border']}]"
_ZERO_TICK = f"bg-[{_P['btn_border']}]"
_PANEL_BG = f"bg-[{_P['card_bg']}]"
_HAIR_RGBA = "rgba({},{},{},1)".format(*theme.hex_rgb(_P["card_border"]))
...
_PANEL = (f"flex-[1_1_300px] min-w-0 px-[30px] py-6 "
          f"shadow-[0_0_0_1px_{_HAIR_RGBA}]")
_FIELD = (f"{theme.MUTED} text-[10px] tracking-[.16em] "
          "uppercase leading-none whitespace-nowrap")
_FIGURE = f"{theme.LABEL} text-[15px] leading-none whitespace-nowrap"
...
def render():
    ...
    with kit.page():
        # No description line: what this board is and how to read it is the
        # opening of page_help.HELP_MD["/sentiment/rotation"].
        head = kit.header("Sector Rotation", view=VIEW, stale=False)
        ...
        # Keeps its runtime text ("RRG vs SPY · as of …"): that is the
        # READING's date, not the page's freshness, which the header stamp
        # answers.
        eyebrow_lbl = kit.status_line()

        # ONE region over the whole board. A refresh replaces the gauge, its
        # axis, the flow band, its footer AND the quadrant panels, and the
        # spinner now lives on the region's OUTER element — so none of the
        # `.clear()` calls below can delete it. That was this page's bug from
        # the 2026-08-17 rebuild until now: `build_busy` mounted the scrim
        # inside `quad_box`, and the build-time `_apply()` → `_paint_quadrants`
        # → `quad_box.clear()` removed it, so Refresh never showed a spinner.
        board = kit.region("Refreshing rotation…")
        with board.content:
            # gap-0: the section rhythm below is the margins', exactly as it
            # was when this body was its own column.
            with ui.column().classes("w-full gap-0"):
                # ── verdict strip ───────────────────────────────────────────
                # gap-px + a 1px ring per panel = a shared hairline that
                # survives wrap. A grid with real borders orphans a rule when a
                # panel reflows.
                with ui.row().classes("w-full flex-wrap gap-px mb-9"):
                ...
                # ── flow band ───────────────────────────────────────────────
                with ui.row().classes(
                        "items-baseline justify-between w-full flex-wrap "
                        "gap-5 mb-3.5"):
                    ...
                    ui.label("Segment width = share of index").classes(
                        f"{theme.MUTED} text-[10px] tracking-[.12em] "
                        "uppercase leading-none")
                ...
                # ── quadrant panels ─────────────────────────────────────────
                with ui.row().classes(
                        "items-stretch w-full no-wrap gap-3.5"):
                    with ui.element("div").classes(
                            "flex items-center w-[22px] shrink-0"):
                        ui.label("RS-Momentum →").classes(
                            f"{theme.MUTED} text-[10px] "
                            "tracking-[.18em] uppercase whitespace-nowrap "
                            "rotate-180 [writing-mode:vertical-rl]")
                    with ui.column().classes("flex-1 min-w-0 gap-0"):
                        # gap over a grid-coloured ground draws the hairlines,
                        # so a panel that reflows can never leave a rule
                        # hanging in space.
                        quad_box = ui.element("div").classes(
                            "grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] "
                            f"gap-0.5 w-full border {_FRAME_EDGE} {_HAIRLINE}")
                        with ui.row().classes(
                                "items-center justify-between w-full no-wrap "
                                f"pt-2.5 {theme.MUTED} text-[10px] "
                ...
                ui.label(
                    "Pairing is ordinal — strongest relative-selling vs "
                    "strongest relative-buying pressure, not literal cash "
                    "flow.").classes(
                    f"{theme.MUTED} text-[10px] tracking-[.1em] "
                    f"uppercase leading-[1.7] mt-[34px] pt-4 border-t "
                    f"{_FRAME_EDGE} w-full")
    ...
    def _paint_gauge(g, threshold):
        ...
        with axis_box:
            for lab in V.gauge_axis(threshold):
                cls = (V.TONE[lab["tone"]]["axis"]
                       if lab["tone"] in ("up", "down") else theme.MUTED)
                ui.label(lab["text"]).classes(
                    f"{cls} text-[9.5px] tracking-[.12em] "
                    "uppercase leading-none whitespace-nowrap")
        if not g:
            return
        tone = V.TONE[g["tone"]]
        with gauge_box:
            ui.element("div").classes(
                f"absolute left-0 right-0 top-2 h-2.5 {_HAIRLINE}")
            ui.element("div").classes(
            ...
            ui.element("div").classes(
                ...
                f"left-[{g['hi_pct']:.2f}%]")
            ui.element("div").classes(
                f"absolute top-0 w-px h-[26px] {_ZERO_TICK} "
            ...
            with foot_box:
                with ui.row().classes(
                    ...
                    ui.label(V.fmt_weight(side["total"])).classes(
                        f"{tone['foot_pct']} text-[20px] "
                        "font-medium leading-none")
                    ui.label(side["label"]).classes(
                        f"{tone['foot_lbl']} text-[10.5px] "
    ...
    def _paint_quadrants(panels):
        ...
        with quad_box:
            for p in panels:
                qc = p["classes"]
                with ui.column().classes(
                        f"{_PANEL_BG} px-[26px] pt-[26px] pb-7 "
                        "min-h-[260px] gap-0"):
                    with ui.row().classes(
