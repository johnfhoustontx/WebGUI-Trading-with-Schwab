...
# ── _num is the house helper, and now literally so ──────────────────────────
# This block used to compare bullbear._num's BODY against four siblings, because
# its docstring asserted the match in prose and nothing enforced prose — a lesson
# this repo had already paid for (the 2026-07-01 audit closed "single-source r"
# as FIXED while gamma_tool still carried five 0.045 literals). On 2026-08-20 the
# six copies were replaced by one import of pages/fmt.num, so identity replaces
# comparison: the pages do not merely AGREE, they are the same object.

def test_every_page_uses_the_one_num_helper():
    from pages import bullbear, desk, momentum_view, rotation_view, rrg_view, sector_heat
    from pages.fmt import num
    for mod, attr in ((bullbear, "_num"), (desk, "_finite"), (momentum_view, "_num"),
                      (rotation_view, "_num"), (rrg_view, "_num"), (sector_heat, "_num")):
        assert getattr(mod, attr) is num, mod.__name__


def test_the_shared_num_still_rejects_what_the_copies_rejected():
    """The behaviour those six copies existed to guarantee, pinned once."""
    from pages.fmt import num
    assert num(True) is None and num(False) is None      # bool subclasses int
    assert num(float("nan")) is None
    assert num(float("inf")) is None
    assert num(None) is None and num("x") is None
    assert num("0.5") == 0.5 and num(0) == 0.0
