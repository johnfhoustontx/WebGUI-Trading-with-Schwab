...
#############################################
# ``TYPICAL_ANALYZE_SEC`` / ``ANALYZE_TIMEOUT_SEC`` / ``analyzing_label`` and
# their four tests moved to ``tests/test_trade_shell.py`` on 2026-09-20, with
# the names themselves. They were written for THIS module's ``render`` -- which
# nothing routes -- while all four reachable Signal Desk screens ran the 30s
# default against the same measured 96s analysis. The tests are unchanged apart
# from the module they read.
