"""Tests for the signal grade/score calibration report.

The question this answers is the only one that makes EV = p*b - (1-p) mean
anything: does the composite score PREDICT? Every `p` the app shows today is
extracted from the option's own price, so EV built on it is ~0 by construction.
The realized outcomes in signals.db are the one independent source of `p` --
and nothing in the repo has ever joined `signals.entry_grade` to
`signal_outcomes.realized_pnl`.
...
class TestSplit:
    ...
    def test_the_rendered_report_names_every_section(self):
        rows = [_row(50.0, scanner_type="0DTE"), _row(50.0, scanner_type="SWING")]
        out = C.format_split(C.split_calibrate(rows, "entry_grade", "scanner_type"),
...
class TestRendering:
    def test_the_table_names_every_bucket_it_was_given(self):
        rows = [_row(50.0, entry_grade="Good")] * 3
        out = C.format_table(C.calibrate(rows, "entry_grade"), "entry_grade")
        assert "Good" in out
