...
# Settings auto-close toggle (read in the loop, default ON). A captured signal's
# stops don't need sub-minute reaction and the cycle fetches up to ~45 chains, so
# 5 min balances freshness vs Schwab-API cost (``_CAPTURED_MANAGE_INTERVAL_MIN`` is the lever).
...
# when the regular session closes. Trading days only. This REPLACED the manual
# account's former 5-min manage cadence. Each hour fires ONCE within a grace
...
# settlement, summarizing the day's result for the paper account. The wide grace tolerates a late/mid-window
...
async def loop(bus):
    ...
    collection; the MANUAL Paper Portfolio runs entry+manage hourly at the top of
    ...
    kill the loop or skip the others."""
    loop_ = asyncio.get_event_loop()
    last_slot = None
    # R6: one-shot startup self-heal — reconcile buying_power_reserved against open
    # positions for the manual account, correcting any BP
    ...
    # republishes it in a ``finally``; this independent call is the backstop so
    # the preview has a book before the first Ledger change whatever happened
    # above. An unchanged book is a skipped write (no version bump, no event).
    try:
        await loop_.run_in_executor(None, handlers.refresh_ledger_caps, bus)
    except Exception:
        log.exception("startup refresh_ledger_caps degraded")
    ...
    while True:
        ...
        # INCOME window — one 30-45 DTE pass a day over the watchlist. The slot is
        # latched in income_ran BEFORE the blocking scan so a slow pass can't
        # double-fire on the next tick (launch_branches' still-running skip means a
        # slow scan only ever delays ITSELF — but only once the slot is marked).
        # The ~23 chain fetches run in the executor; independently guarded so a
        # failure never skips the work above or kills the loop.
        try:
