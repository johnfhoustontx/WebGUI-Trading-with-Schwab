...
def test_slot_does_not_fire_on_a_market_holiday():
    holiday = next(iter(sorted(scheduler._HOLIDAYS)))
...
def test_slot_uses_the_shared_holiday_calendar():
    # Not a private copy — the calendar is maintained in one place per service.
    assert dt.date(2026, 12, 25) in scheduler._HOLIDAYS
