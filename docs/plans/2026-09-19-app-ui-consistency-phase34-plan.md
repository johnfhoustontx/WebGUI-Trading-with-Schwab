...
10. **A render test must be scoped to its own render.** `ui.context.client.elements` is the auto-index client the WHOLE test module shares, so a helper returning `elements.values()` also hands back the spinner another page's kit region left behind — and a scrim test then passes whatever the page under test did. Measured on Task 1: once Task 2 migrated Rotation, the RRG scrim test went green off `test_render_graceful_empty`'s render, and the pre-migration page still passed it. Diff the element ids around the render and return only the new ones. **Prove each new scrim test red against the pre-migration page before implementing** — that mutation is the only thing separating this test from a decoration.
...
- **Four test repairs, each deliberate:**
  1. `test_the_page_imports_nothing_below_tier_one` (111) — add `"pages.ui_kit"` **and `"pages.options"`** (Task 4 found the second is needed too: `from pages.options import theme`, the form all these screens use, records `pages.options`, not the `pages.options.theme` already in the set) to the allowed set. The assertion's intent survives: `ui_kit` is itself Tier 1.
...
- ⚠ **`kit.set_busy` builds no spinner element** — it sets Quasar's `loading` prop and disables the button (measured in Task 4, where the plan had claimed otherwise). So it adds no second spinner to a page, and a test that selects "the first spinner" is not endangered by it.
