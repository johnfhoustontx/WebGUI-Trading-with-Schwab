...
sparkline, and flow alerts) · **Your position** (all three books — account, ledger,
captured — with the rescue flag only where the book carries one). No
Highcharts, the Desk's call.

**Reads** 10 shared views (`options:matrix`, `:scan_funnel`, `:scan_day` via
`read_gated`, `:gex_status`, `:flow_alerts`, the three books, `sentiment:regime`,
...
Trade Analyzer (nav label since 2026-07-11; on-demand single-symbol analysis: **Position (1–8wk)** + **Investor (months+)** Buy/Hold/Sell verdicts w/ score + top reasons + hard gates + expandable factor breakdown. The **Position** verdict is now a **backtested, IC-weighted cross-sectional factor model** (`swing_model.json` artifact → live `swing_model.py` scorer): the headline is the **validated** BUY/SELL/HOLD off a **calibration band** + an outcome line (percentile · expected fwd return / horizon · beat-SPY hit-rate) + a **"Why — validated factors"** evidence expander (per-factor z/weight/contribution/IC + model version & OOS IC), with the **legacy heuristic** verdict tucked into a collapsed expander (Investor unchanged); **MTF EMA alignment** (per-timeframe); momentum strip (RSI/ADX/MACD/VWAP/RelVol); sector strength; **Fundamentals card** (P/E/PEG/growth/ROE/margins via proxy `/instruments`); ~~Markov Forecast card~~ — **REMOVED from the page (2026-06)**: it forecast the LEGACY composite score and so contradicted the validated Position read beside it. `webgui/pages/trade.py` builds no Markov card (nor anything else — its `render` was deleted 2026-09-20, and the four Signal Desk screens are what `main.py` routes); the `markov{}` field survives in the `TradeAnalysis` contract and `cache:trade:markov_prior` is still published, so this is a UI removal, not an engine removal; **dark-navy "dashboard" theme** (the app-wide surface both entrypoints paint under `ns-app`; the page-scoped `.calc-v2` wrap retired 2026-09-20 with `trade.py`’s unrouted `render`, the last element carrying it); **tab-out (`focusout`) = Analyze** (deduped); **persists last analyzed symbol** + analysis across nav. **Deep Dive + AI Query buttons (2026-08-04)** run the migrated **EquityDeepDive** engine (`services/trade_svc/deepdive/`) for the current symbol: **Deep Dive** opens a self-contained HTML report (technicals + fundamentals/short-interest + options analytics [ATM IV, implied move, max pain, 25Δ skew, IV term structure, cm30 IV, net GEX/flip, OI walls] + IV/RV rank) in a new tab via `/trade/deepdive`; **AI Query** opens a copyable chat-prompt (digest injected, no API call) via `/trade/deepdive-query`. On-demand IV history (`repo_paths.IV_HISTORY_DB`), IV rank "building" until snapshots accrue; `ai_analyst.py` NOT migrated — see the 2026-08-04 Last-updated entry)

## The 2026-09-20 kit migration — Phase 6, the system pages (and the migration ends)

User Manuals, Stop All Services, the EOD Report (**both** frames — `render_detail` is
a second `render()` in the same module), System Status, Settings → General and
Settings → Configuration went onto `pages/ui_kit.py`. **That is every page in the
app**: `test_ui_kit_guard`'s `ALLOWED` holds no pending entry any more, only its ten
written exceptions, and `theme.BTN_3D` / `BTN_3D_DANGER` are deleted. ⚠ `TILE_3D` is
NOT a third dead alias — `options/detail.py` mounts it, and it is its own token.

**Widths are a decision, not a default.** The three **Settings** tabs are `kit.page()`
FULL width, because they are peers in one page and a content column that jumps when
you switch tabs is the inconsistency this work removes; **Manuals** and **Stop All
Services** take `kit.page(width="form")`, a widening from `max-w-2xl` (672px) to
`max-w-3xl` (768px).

⚠ **This is the app's destructive surface, and three of its five destructive controls
asked nothing before.** Every one confirms now, through `kit.confirm(danger=True)`:

- **Status → Restart** on nine of eleven cards, including **this web app** and the
  **proxy**. ONE dialog built at page level and retitled per click — never inside
  `comps`, which is cleared and rebuilt every 15 s, and a dialog deletes itself with
  its slot. `restart_body` is PURE and gives three different bodies: a `self` restart
  names the disconnect, a `proxy` restart WHILE THE MARKET IS LIVE names the
  stack-wide outage, everything else says it is back in ~15 s. The market-hours test
  is `config_editor.market_busy`, **imported lazily rather than restated**, so it
  cannot drift from the warning the Configuration tab already shows for the same
  reason; it never raises, and an unreadable calendar degrades to False.
- **EOD → Generate** names the DATE it replaces, set at OPEN time rather than at
  build, because the dialog is built once and a page can outlive midnight. ⚠ A
  refusal **closes** this dialog — the opposite of terminate's refused TOTP, and
  deliberately: there the reader fixes it by typing a better code, here they cannot.
- **Settings → General → Vacuum** now names whether `--purge` is armed
  (`vacuum_body(purge)`, PURE, reading the same switch `vacuum_command(purge)` does).
  ⚠ The work is NOT the confirm's action: `kit.confirm` holds its dialog open until
  `on_confirm` returns, and a VACUUM runs for minutes, so `_start_vacuum` arms the
  page's own button and hands the work to a once-timer in that button's slot.
- **Configuration → Reset all** (per file) is `danger` now, not a quiet grey link:
  it writes `{}` and every override in that file goes.
- **Stop All Services** needed **no guard exception**. `kit.confirm` already had both
  things it looked to be missing: `handle.content` takes the 6-digit field, and an
  `on_confirm` returning **False keeps the dialog open** — which is exactly the rule
  that closing on a refused code would read as *done*. **Enter now confirms** from
  that field, where it did nothing before.

**Two clocks and a stamp.** `status.py` stamped `datetime.now()` and rendered
`%H:%M:%S` with no zone; the kit header's `Updated … CT` replaced it, **driven by
hand** — `kit.header` binds its stamp to a bus view's `:ts` key and this page's
freshness is a PROBE, so the header takes no `view=` (which also keeps it from
registering a poll timer with nothing to read) and the page calls the public
`set_stamp` from the sweep's own clock, LAST and only on a sweep that completed.
⚠ Putting that `set_visibility(True)` inside `kit.set_stamp` was considered and
**rejected**: a kit edit inside a page phase changes every migrated page for one
page's benefit. And `config_store.save` now writes an **aware Central** stamp, which
`config_editor.change_stamp` labels `CT`; the naive rows already in `changes.jsonl`
render unchanged and are deliberately **not** relabelled.

⚠ **Two defects that were not styling.** `config_editor._paint_footer` **cleared and
rebuilt** the footer — the exact thing `appearance.py` carries a note against — so a
Save or Discard clicked straight after editing a value was gone by mouseup
(`AssertionError: Save changes was rebuilt`); it is built once and MUTATED now. And
EOD's Generate called `generate()` with no snapshot and no `has_data` check, so one
click against a stopped stack replaced that day's real report with a 12 KB document
of empty notes — measured, not inferred.

**Off the event loop:** both EOD frames' eight sequential bus reads plus `generate()`
itself, and Status's seven `read_meta` probes in `_paint_freshness` (`_sweep` had
always crossed `run.io_bound`; that one never did, so it ran on the loop every 15 s
in every open tab).

⚠ **Status did NOT have the cleared-container spinner bug** the nine screens above it
had — its spinner was already a sibling of both repainted containers, the one thing
it got right. `kit.region` now wraps each of the two blocks.

**`EOD_CSS` became `build_eod_css(theme)`** and keeps BOTH destinations: `ui.add_css`
in the two frames (this page's one documented escape hatch) and `wrap_document` into
the standalone `summary.html` / `detail.html` that `/eod/file` serves. Those are raw
documents with no NiceGUI and no app stylesheet behind them, so they are
**recoloured, not deleted and not routed through the kit**, and they carry the font
`<link>` or `font-family` would name a face the file never loads. ⚠ `.pos` / `.neg`
were a FOURTH green/red pair. ⚠ One pre-existing problem is left alone: the report's
widest table forces `.eod-report` to 504px, so at 375px the page scrolls sideways —
in the app and in the exported documents alike.

Design: [app-ui-consistency](plans/2026-09-19-app-ui-consistency-design.md); plan:
[phase 6](plans/2026-09-19-app-ui-consistency-phase6-plan.md).
...
wrapper is a neutral `ns-app w-full p-4 gap-3` column — deliberately NOT `theme.PAGE`.
⚠ **The REASON changed with the kit migration and the wrapper did not.** It used to be
that each page supplied its own top-level wrap and background (`CONSOLE_PAGE`,
`RT_VOID_BG`, the Macro Board's radial, `calc-v2 PAGE`) and a second one would draw a
frame around it. All four of those are retired, and every page now opens with
`kit.page()`, which supplies a column width and nothing else — the app surface comes
from `theme.SURFACE_CSS` under `ns-app`, injected below. `theme.PAGE` here would
therefore draw a bordered, gradient-filled PANEL around each screen on top of a page
that is already painted. What
...
`/desk` renders merged paper and captured positions with rescue flags, `/opportunity`
