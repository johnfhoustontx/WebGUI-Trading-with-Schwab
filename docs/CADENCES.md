...
|---|---|---|---|---|---|
| Market summary (ticker + Desk MARKET SUMMARY frame) | market_svc | **change-driven, not clocked** (2026-09-10) — written when the six-reading fingerprint moves — the Bull/Bear sector counts compared with a one-sector tolerance (`FINGERPRINT_SECTOR_TOLERANCE`) — never twice within `SUMMARY_MIN_GAP_SEC` (10 min), never past `SUMMARY_DAILY_CAP` (30/day); a **withheld** reply keeps its fingerprint, so only a failed call is retried | `summary_due`; the Settings **ticker toggle** only hides the marquee and no longer gates the call (both surfaces read the one sentence) | Sonnet 5 | `market_svc/scheduler.py` `SUMMARY_MIN_GAP_SEC`/`SUMMARY_DAILY_CAP`; `compute.summary_fingerprint`/`same_summary_readings` |
...
Measured on prod 2026-09-10: **~20 calls/weekday** — then including the autonomous
driver's checkpoints every 30 min 08:45–14:30 CT, removed 2026-09-22, which leaves
the 4 briefings with the ticker narrative OFF since
...
| Page version polls (Gamma, tables, …) | 2 s | Gamma coalesces all views into one pipelined `read_versions` |
