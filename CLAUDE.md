...
wrong, caught during the build) → **`decider.decide`** (Claude **Sonnet 4.6** via the
...
- **Decider** `decider.py`: `build_packet`'s model-facing prompt + a forced
  `submit_decision` tool-use call to **Claude Sonnet 4.6** (`anthropic` SDK, LAZY import,
...
- **Contracts** `DriverControl` (`cache:driver:control` — master switch + STOP latch)
  ...
  `MAX_CONCURRENT=6`, `MAX_TRADES_PER_CYCLE=3`, `MODEL="claude-sonnet-4-6"`,
  `CHECKPOINT_MIN=30`); the daily loss cap is sourced from the legacy
  `config.RISK_LIMITS`.
