...
| `/desk/live` | **Desk (streaming mirror)** — the same screen as a STANDALONE HTML document + an SSE stream (`/desk/stream`): no NiceGUI, no websocket, so a wall display or a phone reconnects with an HTTP request. `webgui/desk_stream.py` — `snapshot()` is PURE and returns display-ready **strings** built by `pages/desk.py`'s OWN builders; the browser only places colour. ⚠ That delegation covered the ROWS only until 2026-09-04: the panel titles were HTML literals here, two subtitles were rebuilt with a local `.format`, all four column-label lists and all five empty-state strings were byte-copies — so a reword could reach one screen and leave the other naming the same number differently. Both now read `desk.PANEL_HEADS`, the four `*_HEADS` label tuples and the `EMPTY_*` / `stale_walls_note` constants. **`_JS` is inserted verbatim and can never be `.format`ed** (it is full of braces), so anything it needs arrives through the `consts` injection — the route `CALL_HEX` already took. Two event types, deliberately on separate cadences: `desk` on a cache-version change (2 s probe), `clock` every second (which doubles as the SSE keep-alive). Panels are **always 2x2**, as `/desk` has been since 2026-08-20 — this screen gets pinned to a display, so it must not reflow. ⚠ Outside the Tailwind-first standard — a raw `HTMLResponse` document, the same out-of-scope case as the EOD report. | built |
